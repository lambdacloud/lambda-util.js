    ...
    var req = request
      .put(httpOptions, function(err, res) {
        ...
        if (op.retry(err)) {
          ...
          if (res) {
            fs.appendFile('retry.log', 'Retry puting message with status code' + chunk + ' ' + currentAttempt + ' times res.statusCode: ' + res.statusCode + '\n', function (error) {
              if (error) throw error;
              //console.log('append successfully');
            });
          } else {
            fs.appendFile('retry.log', 'Retry puting message without status code' + chunk + ' ' + currentAttempt + ' times\n', function (error) {
              if (error) throw error;
              //console.log('append successfully');
            });
          }
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          var endTime = moment();
          fs.appendFile('time.log', 'httpoptions ' + JSON.stringify(httpOptions) + ' time: ' + endTime.diff(beginTime) + '\n', function (err) {
            if (err) throw err;
            //console.log('append successfully');
          });
        } else {
