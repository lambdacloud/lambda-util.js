...
JsonAddFieldParser.prototype._transform = function transformJsonAddField(data, encoding, callback) {
  ...
  //if data._source['timestamp'] match reg then change into 30 in the end
  if (reg.test(data._source['timestamp'])) {
    data._source['timestamp'] = (data._source['timestamp']).match(reg)[1]+':30';
  }
  //if
